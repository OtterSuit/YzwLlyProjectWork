<template>
  <div class="receive-container">
    <!-- header -->
    <myfilters title="签收单" :back-button="true" />
    <!-- header end -->
    <div class="main-box">
      <!-- form -->
      ...
      <el-table v-loading="listLoading" :data="orderPackets" style="width: 100% ">
        ...
        <el-table-column label="状态">
          <template slot-scope="scope">
            <div v-if="scope.row.complete===0" class="base-color"><i class="el-icon-time" />待签收</div>
            <div v-else-if="scope.row.complete===scope.row.packetCount" class="success-color"><i class="el-icon-success" />签收完成</div>
            <div v-else class="goon-color"><i class="icon-fahuo iconfont" />正在签收，已完成{{ scope.row.complete }}包</div>
          </template>
        </el-table-column>
      </el-table>
      <!-- table end -->
    ...
    <div class="main-box">
      <div class="main-box-title">签收信息</div>
      <div class="main-box-receive">
        <el-form ref="receiveForm" :model="receiveForm" label-width="80px" :rules="rules">
          <el-form-item label="签收科室" style="width:50%">
            <el-input v-model="detailForm.applyDeptName" disabled />
          </el-form-item>
          <el-form-item label="签收人" style="width:50%" prop="receiveman">
            <el-input v-model="receiveForm.receiveman" placeholder="请输入或扫描工牌" />
          </el-form-item>
        </el-form>
      </div>
    </div>
    <el-button :loading="buttonLoading" type="primary" style="width:145px; height: 50px" @click="receiveSubmit()">确定签收</el-button>
    <!-- 签收弹出框 -->
    <el-dialog v-el-drag-dialog :visible.sync="receiveShow" width="95%">
      <template slot="title">
        ...
        <el-scrollbar class="scrollbar">
          <div class="dialog-packet">
            <el-form ref="form" :model="form" :rules="rules" @submit.native.prevent>
              <el-form-item prop="packetInstanceId">
                <el-input
                  ref="inInputs"
                  v-model.trim="form.packetInstanceId"
                  type="tel"
          ...
          <el-table :data="packetInstanceIds" style="100%" :show-header="false">
            ...
            <el-table-column prop="packetInstanceId" width="260px" />
            <el-table-column prop="name" />
            <el-table-column>
              <template slot-scope="scope">
                <div v-if="instanceIds.includes(scope.row.packetInstanceId)" class="success-color"><i class="el-icon-success" />已签收</div>
                <div v-else class="base-color"><i class="el-icon-time" />待签收</div>
              </template>
            </el-table-column>
          </el-table>
          <!-- table end -->
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="receiveShow=false">确定</el-button>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        receiveman: [
          { required: true, message: '请输入签收人', trigger: ['change', 'blur'] }
        ],
        packetInstanceId: [
  ...
  methods: {
    fetchData() {
      ...
      api.toGetWorkorder({ id: this.id }).then(response => {
        if (response.code === '200' && response.data.busiCode === '1') {
          ...
          this.packetInstanceIds = response.data.packetInstancePage.records
          // 签收信息
    ...
    // 检查输入id长度
    checkInput() {
      if (this.form.packetInstanceId.length >= 15) {
        this.getId()
      }
    },
    // 扫码或输入编号
    getId() {
      this.$refs.form.validate(async valid => {
        if (valid) {
          ...
          this.form = {}
          formObj.packetInstanceId = formObj.packetInstanceId.trim() // 去除留空
          if (this.instanceIds.includes(formObj.packetInstanceId)) {
            this.$message({
              message: '该包已签收',
              type: 'warning'
            })
          ...
          this.packetInstanceIds.forEach(item => {
            if (item.packetInstanceId === formObj.packetInstanceId) {
              bool = true
              this.instanceIds.push(formObj.packetInstanceId)
              this.orderPackets.forEach(elem => {
                if (elem.packetDefineId === item.packetDefineId) {
                  elem.complete += 1
                }
              })
              return
    ...
    receiveSubmit() {
      ...
      this.$refs.receiveForm.validate(async valid => {
        if (valid) {
          ...
          const receive = {
            id: this.id,
            packetInstanceIds: this.instanceIds,
            receiveUserid: this.receiveForm.receiveman
          }
          this.buttonLoading = true
          // 发起请求
          api.toReceiveGoods(receive).then(response => {
            if (response.code === '200' && response.data.busiCode === '1') {
              this.$router.go(-1)
