<template>
  <div class="delivery-container">
      ...
      <el-table v-loading="listLoading" :data="orderPackets" style="width: 100% ">
        ...
        <el-table-column width="150px">
          <template slot-scope="scope">
            <el-button size="mini" icon="el-icon-tickets" @click="detailShow(scope.$index,scope.row)">查看详细</el-button>
          </template>
        </el-table-column>
      </el-table>
      <!-- table end -->
    ...
    <div class="main-box">
      ...
      <!-- table -->
      <el-table :data="orderTraces" style="width: 100% ">
        <el-table-column label="序号" type="index" width="100px" />
    ...
    fetchData() {
      ...
      api.toGetWorkorder({ id: this.id }).then(response => {
        if (response.code === '200' && response.data.busiCode === '1') {
          ...
          if (this.countingstatus.includes(this.detailForm.currentStatus)) {
            if (this.detailForm.workorderType === '2') {
              this.detailForm.status = this.ssd_workorder_status['5']
              console.log(this.detailForm.status)
            } else {
              this.detailForm.status = this.ssd_workorder_status['7']
            }
          } else {
