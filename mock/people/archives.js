  ...
  'items|194': [
    {
      ...
      'skill|1-3': [
        {
          key: /[a-z][A-Z][0-9]/,
          'certificateName|1': ['资格证书', '护士执业证书', '助产证'],
          'level|1': ['初级', '中级', '高级'],
...
module.exports = [
  // 获取用户个人档案
  {
    ...
    response: config => {
      ...
      const items = data.items.find(item => item.id === config.query.id)
      return {
        code: 20000,
        data: items
      }
  ...
  // 获取id
  {
    url: '/vue-admin-template/people/userId',
    type: 'get',
    response: config => {
